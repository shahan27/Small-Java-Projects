...
public class SpriteSheet {
    ...
    {
        if (spritesLoaded) {
            int spritesID = x + y * (SIZEX /spriteSizeX);

            if(spritesID < loadedSprites.length)
                return loadedSprites[spritesID];
            else
                System.out.println("SPriteID of" +spritesID+ "is out of range with a maximum of" + loadedSprites.length + ".");
        } else
